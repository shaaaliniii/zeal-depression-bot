...
# Provide the absolute path to 'concerning_words.txt'
# concerning_words_file_path = r"C:\Users\Shalini Singh\Downloads\TEDDY-main\TEDDY-main\concerning_words.txt"
concerning_words_file_path = "concerning_words.txt"

danger_words.load_danger_words(concerning_words_file_path)
...
def TEDDY(essay, progress, filename):
    if essay != "":
        ...
        for sentence in sentences:
            loading.progress(i/len(sentences), text=f"Analyzing sentence {i}/{len(sentences)} in file {progress}...")
        ...
        loading.progress(1, text=f"Completed file {progress}.")
...
for uploaded_file in uploaded_files:
    ...
    string += TEDDY(GetPDFText(current_pdf), f"{i}/{len(uploaded_files)}", current_pdf.stream.name)
